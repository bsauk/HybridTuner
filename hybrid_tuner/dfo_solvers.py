class dfoClass():
    ...
    def call_dfo_7(self):
        ...
        f.write('prt = 1;\n')
        f.write('smax = 5*n+10;\n')
        f.write('[x_best, f_best, xmin, fmi, ncall, ncloc] = mcs(fcn, data, bl, bu, prt, smax, budget);\n')        
        f.close()
    ...
    def call_dfo_8(self):
        ...
        constraint_id=0
        for i,j in zip(range(0, self.tune.nvars), range(0, 2*self.tune.nvars, 2)):
            fconst.write('c_const(' + str(j+1) + ') = bl(' + str(i+1) + ') - x(' + str(i+1) + ');\n')
            fconst.write('c_const(' + str(j+2) + ') = x(' + str(i+1) + ') - bu(' + str(i+1) + ');\n')
        ...
        while counter < self.tune.nvars:
            ...
            while scounter < self.tune.nvars:
                scounter = scounter + 1
                fgrad.write("grad_c(" + str(scounter) + "," + str(constraint_id) + ") = 0;\n")
            fgrad.write("grad_c(" + str(counter) + "," + str(constraint_id) + ") = -1;\n")
            constraint_id=constraint_id+1
            scounter=0
            while scounter < self.tune.nvars:
                scounter = scounter + 1
        ...
        fp.write("phi = 1;\n")
        fp.write("theta = 0.5;\n")
        fp.write("fevals_max = " + str(self.tune.limit_evals) + ";\n")
        fp.write("iter_max = " + str(self.tune.limit_evals) + ";\n")
    ...
    def call_dfo_10(self):
        ...
        f.write('"Maximum Evaluations" int '" " + str(self.tune.limit_evals) + "      \n")
        f.write('"Display"   int 0      \n')
        f.write('@@\n')
        f.write('@ "Citizen 1"\n')
        ...
        fcall.write(self.tune.dfo_path + '/HOPSPACK_main_serial hopspack_input.in > 10.results &')
        fcall.close()
        f.close()
    ...
    def call_dfo_15(self):
        ...
        f.write("fcn = 'func_f';\n")
        f.write('fac = 0;\n')
        f.write('ncall = ' + str(self.tune.limit_evals) + ';\n')
        f.write('u = bl;\n')
        f.write('v = bu;\n')
        f.write('n = ' + str(self.tune.nvars) + ';\n')
        f.write('npoint = 1;\n')
        ...
        f.write('   f(j,:) = [feval(fcn, x(j,:))+fac*randn max(sqrt(eps), 3*fac)];\n')
        f.write('end\n')
        f.write('ncall0 = npoint;\n')
        f.write("params = struct('bounds', {u,v}, 'nreq', nreq, 'p', p);\n")
        f.write('% repeated calls to Snobfit\n')
        f.write('while ncall0 < ncall %repeat till ncall function values are reached\n')
        f.write('   if ncall0 == npoint % intial call\n')
        f.write('      [request, xbest, fbest] = snobfit(file, x, f, params, dx);\n')
        f.write('      ncall0, xbest, fbest\n')
        f.write('   else\n')
        f.write('      [request, xbest, fbest] = snobfit(file, x, f, params);\n')
        f.write('   end\n')
        ...
        f.write('      ncall0, xbest, fbest % Display Current number of function values, best point and function values if fbest has changed\n')
        f.write('   end\n')
        f.write('end\n')
        f.write('ncall0, xbest, fbest %show number of function values, best point and function value\n')
